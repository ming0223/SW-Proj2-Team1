...
class Calculator(QWidget):
    ...
    def buttonClicked(self):
        ...
        if key == '=':
            ...
            arr = ['3.141592', '3E+8', '340', '1.5E+8']
            self.display.setText(self.display.text() + str(arr[constantList.index(key)]))

        elif key in functionList:
            n = self.display.text()
            arr = ['factorial', 'decToBin', 'binToDec', 'decToRoman']
            for i in range(0, 4) :
                if key == functionList[i] :
                    value = getattr(calcFunctions, arr[i])(n)
